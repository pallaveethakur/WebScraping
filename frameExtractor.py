...
dire=r'C:\Users\User\Desktop\tiffin_V_F'
vedios=os.listdir(dire+'/'+'V_kitchen')
for ind,vedio in enumerate(vedios):
    try:
        path=dire+'/'+'V_kitchen'+'/'+vedio
        fe = FrameExtractor(path)
        fe.get_video_duration()
        fe.get_n_images(every_x_frame=10)
        fe.extract_frames(every_x_frame=10, img_name=str(ind)+'kitchen',dest_path=dire+'/'+'F_kitchen')
